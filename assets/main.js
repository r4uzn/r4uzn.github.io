...
import { categories, posts } from './posts.generated.js';
...
function highlightedHeadline(value) {
...
function renderInlineMarkdown(value) {
    ...
    html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" loading="lazy" referrerpolicy="no-referrer" />');
...
function renderMarkdown(markdown) {
    ...
    const flushCode = () => {
        if (code.length || inCode) {
            chunks.push(`<pre><code data-lang="${escapeHtml(codeLang)}">${escapeHtml(code.join('\n'))}</code></pre>`);
            code = [];
            codeLang = '';
        }
    };
...
function initHomeMotion() {
    ...
    const title = document.querySelector('#home-section-title');
    const count = document.querySelector('#home-section-count');
    if (!targets.length) {
        return () => { };
    }
    let revealObserver;
...
function initPageMotion(routeName) {
